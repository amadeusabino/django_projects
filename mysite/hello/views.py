...
def myview(request) :
    # set cookie
    #oldval = request.COOKIES.get('amd', None)
    #resp = HttpResponse('In a view - the amd cookie value is '+str(oldval))
    #if oldval :
    #    resp.set_cookie('amd', int(oldval)+1) # No expired date = until browser close
    #else :
    #    resp.set_cookie('amd', 10) # No expired date = until browser close
    #resp.set_cookie('alsoft', 10, max_age=1000) # seconds until expire
    ...
    resp.set_cookie('dj4e_cookie', 'b9806a66', max_age=1000)
